...
const cliente_1 = require("../../dominio/models/cliente");
...
const createClient = async (req, res) => {
    ...
    const documentAlreadyExist = await cliente_1.Cliente.findOne({
        where: {
            [sequelize_1.Op.and]: [{ idType: idType }, { documentNumber: documentNumber }],
        },
    });
    if (documentAlreadyExist) {
        ...
        return res
            .status(409)
            .json(`The document number ${documentNumber} is already exist for the type ${idType}`);
    }
    try {
...
const getClient = async (req, res) => {
    ...
    if (id) {
        ...
        if (!client) {
            return res
                .status(404)
                .json(`The client with document number ${documentNumber} doesn't exist`);
        }
        return res.status(201).json(client);
    }
};
