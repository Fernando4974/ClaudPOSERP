import { User } from "../../dominio/models/user";
...
export const requestPasswordReset = async (req: Request, res: Response) => {
  try {
    ...
    const mailOptions = {
      ...
      html: `http://localhost:${
        process.env.PORTBACKEND
      }/resetPassword?email=${email}&token=${jwt.sign(
        { email },
        process.env.SECRET_KEY || "890sfd798s56423jk",
        { expiresIn: "15m" }
      )}`,
  ...
  try {
    ...
    try {
      ...
      if (verifyPassawords) {
        ...
        return res
          .status(409)
          .json({ msg: `The password can not be the same as previous one` });
      }
    } catch (error) {
      res.status(409).json({
