...
export const createClient = async (req: Request, res: Response) => {
  ...
  const documentAlreadyExist: any = await Cliente.findOne({
    where: {
      [Op.and]: [{ idType: idType }, { documentNumber: documentNumber }],
    },
  });

  if (documentAlreadyExist) {
    console.log(
      `The document number ${documentNumber} is already exist for the type ${idType}`
    );
    return res
      .status(409)
      .json(
  ...
  if (id) {
    ...
    if (!client) {
      return res
        .status(404)
        .json(
          `The client with document number ${documentNumber} doesn't exist`
        );
    }
